...
from Vizxer-m import collection, user_collection, application
...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    if not user:
        if update.message:
            ...
            await update.callback_query.edit_message_text('ɢᴇᴇᴢ ! ɢᴏ ɢᴇᴛ ᴀᴛʟᴇᴀsᴛ ᴏɴᴇ ᴡᴀɪғᴜ ᴀɴᴅ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ')
        return
    ...
    if 'favorites' in user and user['favorites']:
        ...
        if user['characters']:
            ...
            if update.message:
                await update.message.reply_text("ʙᴀᴋᴀ ᴏʙᴛᴀɪɴ sᴏᴍᴇ ᴡᴀɪғᴜs ᴄᴜᴢ ʏᴏᴜʀ ʟɪsᴛ ɪs ᴇᴍᴘᴛʏ")
...
async def harem_callback(update: Update, context: CallbackContext) -> None:
    ...
    if query.from_user.id != user_id:
        await query.answer("ʙᴀᴋᴀ ᴅᴏɴ'ᴛ sɪᴍᴘ ᴏɴ ᴏᴛʜᴇʀ's ʜᴀʀᴇᴍ", show_alert=True)
        return
